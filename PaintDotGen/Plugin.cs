...
internal sealed class Plugin() : CpuRenderingPluginBase<Settings>(new PluginInfo()) {
  internal enum PropertyNames { Seed, Complexity, ConstantRange, ImaginedCanvasRescaleFactor, Normalized, Interpreter }
  ...
  protected override void OnPixelRender(IRenderingContext<ColorBgra32> context, Settings settings, Vector<int> position) 
    => context.Draw(position, settings.Interpreter(Expressions, position.As<float>() * settings.RescaleFactor));
  ...
  protected override void OnSettingsChanged(Settings oldSettings, Settings newSettings, bool firstChange) {
    ...
    var generatorContext = new ExpressionFactoryContextBuilder()
      .FromSettings(newSettings, new Size<int>(Environment.Document.Size.Width, Environment.Document.Size.Height))
      .Build();
